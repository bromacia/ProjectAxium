    ...
    public:
        ...
        class spell_gen_cannibalize_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckIfCorpseNear()
            {
                ...
                Axium::AnyDeadUnitSpellTargetInRangeCheck check(caster, max_range, GetSpellInfo(), TARGET_SELECT_CHECK_ENEMY);
                Axium::WorldObjectSearcher<Axium::AnyDeadUnitSpellTargetInRangeCheck> searcher(caster, result, check);
...
class spell_pvp_trinket_wotf_shared_cd : public SpellScriptLoader
{
    public:
        ...
        class spell_pvp_trinket_wotf_shared_cd_SpellScript : public SpellScript
        {
            ...
            void Register()
            {
                OnEffectLaunch += SpellEffectFn(spell_pvp_trinket_wotf_shared_cd_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };
                ...
                switch (GetSpellInfo()->Id)
                {
                    ...
                    case SPELL_COPY_WEAPON_3:
                    {
                        if (Player* player = caster->ToPlayer())
                        {
                            ...
                            {
                                if (uint32 mainTransmogEntry = mainItem->TransmogEntry)
                                    target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID, mainTransmogEntry);
                                else
                    ...
                    case SPELL_COPY_OFFHAND_2:
                    {
                        if (Player* player = caster->ToPlayer())
                        {
                            ...
                            {
                                if (uint32 offTransmogEntry = offItem->TransmogEntry)
                                    target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 1, offTransmogEntry);
                                else
                    ...
                    case SPELL_COPY_RANGED:
                    {
                        if (Player* player = caster->ToPlayer())
                        {
                            ...
                            {
                                if (uint32 rangedTransmogEntry = rangedItem->TransmogEntry)
                                    target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 2, rangedTransmogEntry);
                                else
