...
void OPvPCapturePointZM_Beacon::ChangeState()
{
    ...
    if (m_OldState == OBJECTIVESTATE_ALLIANCE)
    {
        ...
        sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetAxiumStringForDBCLocale(ZMBeaconLoseA[m_TowerType]));
    }
    ...
    else if (m_OldState == OBJECTIVESTATE_HORDE)
    {
        ...
        sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetAxiumStringForDBCLocale(ZMBeaconLoseH[m_TowerType]));
    }
    ...
    switch (m_State)
    {
        case OBJECTIVESTATE_ALLIANCE:
        {
            ...
            if (alliance_towers < ZM_NUM_BEACONS)
                ((OutdoorPvPZM*)m_PvP)->SetAllianceTowersControlled(++alliance_towers);
            sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetAxiumStringForDBCLocale(ZMBeaconCaptureA[m_TowerType]));
            break;
        }
        case OBJECTIVESTATE_HORDE:
        {
            ...
            if (horde_towers < ZM_NUM_BEACONS)
                ((OutdoorPvPZM*)m_PvP)->SetHordeTowersControlled(++horde_towers);
            sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetAxiumStringForDBCLocale(ZMBeaconCaptureH[m_TowerType]));
            break;
        }
    ...
    if (retval >= 0)
    {
        if (player->HasAura(ZM_BATTLE_STANDARD_A) && m_GraveYardState != ZM_GRAVEYARD_A)
        {
            if (m_GraveYardState == ZM_GRAVEYARD_H)
                sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetAxiumStringForDBCLocale(LANG_OPVP_ZM_LOSE_GY_H));
            ...
            player->RemoveAurasDueToSpell(ZM_BATTLE_STANDARD_A);
            sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetAxiumStringForDBCLocale(LANG_OPVP_ZM_CAPTURE_GY_A));
        }
        else if (player->HasAura(ZM_BATTLE_STANDARD_H) && m_GraveYardState != ZM_GRAVEYARD_H)
        {
            if (m_GraveYardState == ZM_GRAVEYARD_A)
                sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetAxiumStringForDBCLocale(LANG_OPVP_ZM_LOSE_GY_A));
            ...
            player->RemoveAurasDueToSpell(ZM_BATTLE_STANDARD_H);
            sWorld->SendZoneText(ZM_GRAVEYARD_ZONE, sObjectMgr->GetAxiumStringForDBCLocale(LANG_OPVP_ZM_CAPTURE_GY_H));
        }
        UpdateTowerState();
    }
    return retval;
