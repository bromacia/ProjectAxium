...
namespace MMAP
{
    ...
    bool MMapManager::loadMap(const std::string& basePath, uint32 mapId, int32 x, int32 y)
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMAP:loadMap: Bad header in mmap %03u%02i%02i.mmtile", mapId, y, x);
            return false;
        }

        if (fileHeader.mmapVersion != MMAP_VERSION)
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMAP:loadMap: %03u%02i%02i.mmtile was built with generator v%i, expected v%i",
        ...
        if (!result)
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMAP:loadMap: Bad header or data in mmap %03u%02i%02i.mmtile", mapId, y, x);
            fclose(file);
            return false;
        }
        ...
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMAP:loadMap: Could not load %03u%02i%02i.mmtile into navmesh", mapId, y, x);
            dtFree(data);
            return false;
        }
    ...
    bool MMapManager::unloadMap(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (status != DT_SUCCESS)
        {
            ...
            sLog->outDebug(LOG_FILTER_MAPS, "MMAP:unloadMap: Could not unload %03u%02i%02i.mmtile from navmesh", mapId, y, x);
            ASSERT(false);
        }
        else
        {
    ...
    dtNavMeshQuery const* MMapManager::GetNavMeshQuery(uint32 mapId, uint32 instanceId)
    {
        ...
        if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end())
        {
            ...
            {
                dtFreeNavMeshQuery(query);
                sLog->outDebug(LOG_FILTER_MAPS, "MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for mapId %03u instanceId %u", mapId, instanceId);
                return NULL;
            }
