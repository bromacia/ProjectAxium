...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    if (sWorld->IsFFAPvPRealm() && !pCurrChar->HasGameMasterTagOn() && !pCurrChar->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING))
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recv_data)
{
    ...
    if (recv_data.GetOpcode() == CMSG_CHAR_FACTION_CHANGE)
    {
        ...
        for (std::map<uint32, uint32>::const_iterator itr = sObjectMgr->FactionChange_Items.begin(); itr != sObjectMgr->FactionChange_Items.end(); ++itr)
        {
            ...
            trans->PAppend("UPDATE `item_instance` ii, `character_inventory` ci SET ii.TransmogEntry = '%u' WHERE ii.TransmogEntry = '%u' AND ci.guid = '%u' AND ci.item = ii.guid", team == BG_TEAM_ALLIANCE ? item_alliance : item_horde, team == BG_TEAM_ALLIANCE ? item_horde : item_alliance, guid);
        }
