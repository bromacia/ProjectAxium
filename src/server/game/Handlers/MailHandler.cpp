...
void WorldSession::HandleSendMail(WorldPacket & recv_data)
{
    ...
    {
        SendNotification(GetAxiumString(LANG_MAIL_SENDER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
    ...
    {
        SendNotification(GetAxiumString(LANG_MAIL_RECEIVER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
            ...
            if (!AccountMgr::IsPlayerAccount(GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
            {
                ...
                else
                {
                    ...
                    if (!sObjectMgr->GetPlayerNameByGUID(sender_guid, sender_name))
                        sender_name = sObjectMgr->GetAxiumStringForDBCLocale(LANG_UNKNOWN);
                }
