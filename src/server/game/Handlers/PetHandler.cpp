...
void WorldSession::HandlePetActionHelper(Unit* pet, uint64 guid1, uint16 spellid, uint16 flag, uint64 guid2)
{
    ...
    switch (flag)
    {
                    ...
                    if (pet->getVictim() != TargetUnit || (pet->getVictim() == TargetUnit && !pet->GetCharmInfo()->IsCommandAttack()))
                    {
                        ...
                        if (pet->GetTypeId() != TYPEID_PLAYER && pet->ToCreature()->IsAIEnabled)
                        {
                            ...
                            if (pet->ToCreature()->isPet() && ((Pet*)pet)->getPetType() == SUMMON_PET && pet != TargetUnit && urand(0, 100) < 10)
                                ...
                                pet->SendPetAIReaction(guid1); // 90% chance for pet and 100% chance for charmed creature
                        }
                        else                                // charmed player
                        {
        ...
        case ACT_ENABLED:                                   // 0xC1    spell
        {
            ...
            charmInfo->SetIsAtStay(false);
            if (pet->ToCreature()->GetReactState() != REACT_PASSIVE)
                charmInfo->SetIsCommandAttack(true);
            charmInfo->SetIsReturning(false);
            charmInfo->SetIsFollowing(false);
...
bool WorldSession::CheckStableMaster(uint64 guid)
{
    ...
    {
        if (!GetPlayer()->HasGameMasterTagOn() && !GetPlayer()->HasAuraType(SPELL_AURA_OPEN_STABLE))
        {
