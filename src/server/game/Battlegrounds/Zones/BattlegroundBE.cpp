...
{
    for (uint8 i = BG_BE_OBJECT_DOOR_1; i <= BG_BE_OBJECT_DOOR_4; ++i)
        SpawnBGObject(i, RESPAWN_IMMEDIATELY);

    for (uint8 i = BG_BE_OBJECT_BUFF_1; i <= BG_BE_OBJECT_BUFF_2; ++i)
        SpawnBGObject(i, RESPAWN_ONE_DAY);
}

void BattlegroundBE::StartingEventOpenDoors()
{
    for (uint8 i = BG_BE_OBJECT_DOOR_1; i <= BG_BE_OBJECT_DOOR_2; ++i)
        DoorOpen(i, true);
...
void BattlegroundBE::AddPlayerToScoreboard(Player* player, uint32 team)
{
...
void BattlegroundBE::HandleAreaTrigger(Player* Source, uint32 Trigger)
{
    ...
    switch (Trigger)
    {
        ...
        case 4539:                                          // buff trigger?
            ...
            break;
        default:
            sLog->outError("WARNING: Unhandled AreaTrigger in BattlegroundBE: %u", Trigger);
...
bool BattlegroundBE::SetupBattleground()
{
    ...
    // Ready Markers
    if (!AddObject(BG_BE_OBJECT_READY_MARKER_A, BG_OBJECTID_READY_MARKER, 6293.16f, 283.2f, 5.44f, 0.85f, 0, 0, 0, 0, RESPAWN_IMMEDIATELY) ||
        !AddObject(BG_BE_OBJECT_READY_MARKER_B, BG_OBJECTID_READY_MARKER, 6183.76f, 240.61f, 5.42f, 4.0f, 0, 0, 0, 0, RESPAWN_IMMEDIATELY))
    {
        sLog->outErrorDb("BatteGroundBE: Failed to spawn Ready Markers Battleground not created!");
        return false;
    }
