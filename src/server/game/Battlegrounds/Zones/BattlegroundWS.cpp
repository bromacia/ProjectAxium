...
void BattlegroundWS::PostUpdateImpl(uint32 diff)
{
    ...
    {
        if (GetDuration() >= 25 * MINUTE * IN_MILLISECONDS)
        {
        ...
        else if (GetDuration() > uint32(m_minutesElapsed * MINUTE * IN_MILLISECONDS))
        {
            ++m_minutesElapsed;
...
{
    for (uint8 i = BG_WS_OBJECT_DOOR_A_1; i <= BG_WS_OBJECT_DOOR_H_4; ++i)
    {
        DoorClose(i);
        SpawnBGObject(i, RESPAWN_IMMEDIATELY);
    }
    for (uint8 i = BG_WS_OBJECT_A_FLAG; i <= BG_WS_OBJECT_BERSERKBUFF_2; ++i)
...
void BattlegroundWS::AddPlayerToScoreboard(Player* player, uint32 team)
{
...
void BattlegroundWS::HandleAreaTrigger(Player* Source, uint32 Trigger)
{
    ...
    switch (Trigger)
    {
        ...
        case 4629:                                          // unk4
            break;
        default:
            sLog->outError("WARNING: Unhandled AreaTrigger in BattlegroundWS: %u", Trigger);
...
bool BattlegroundWS::SetupBattleground()
{
    ...
    // Ready Markers
    if (!AddObject(BG_WS_OBJECT_READY_MARKER_A, BG_OBJECTID_READY_MARKER, 1505.72f, 1487.54f, 352.0f, 6.23f, 0, 0, 0, 0, RESPAWN_IMMEDIATELY) ||
        !AddObject(BG_WS_OBJECT_READY_MARKER_B, BG_OBJECTID_READY_MARKER, 948.86f, 1429.22f, 345.38f, 3.12f, 0, 0, 0, 0, RESPAWN_IMMEDIATELY))
    {
        sLog->outErrorDb("BatteGroundWS: Failed to spawn Ready Markers Battleground not created!");
        return false;
    }
