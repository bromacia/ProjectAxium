...
void BattlegroundEY::StartingEventCloseDoors()
{
    ...
    for (uint8 i = BG_EY_OBJECT_A_BANNER_FEL_REAVER_CENTER; i < BG_EY_OBJECT_MAX; ++i)
        if (i != BG_EY_OBJECT_READY_MARKER_A && i != BG_EY_OBJECT_READY_MARKER_B)
...
void BattlegroundEY::HandleAreaTrigger(Player* Source, uint32 Trigger)
{
    ...
    switch (Trigger)
    {
        ...
        case 5866:
            break;
        default:
            sLog->outError("WARNING: Unhandled AreaTrigger in BattlegroundEY: %u", Trigger);
...
bool BattlegroundEY::SetupBattleground()
{
    ...
    // Ready Markers
    if (!AddObject(BG_EY_OBJECT_READY_MARKER_A, BG_OBJECTID_READY_MARKER, 2516.79f, 1596.71f, 1266.19f, 0.0f, 0, 0, 0, 0, RESPAWN_IMMEDIATELY) ||
        !AddObject(BG_EY_OBJECT_READY_MARKER_B, BG_OBJECTID_READY_MARKER, 1814.5f, 1539.53f, 1264.52f, 0.0f, 0, 0, 0, 0, RESPAWN_IMMEDIATELY))
    {
        sLog->outErrorDb("BatteGroundEY: Failed to spawn Ready Markers Battleground not created!");
        return false;
    }
