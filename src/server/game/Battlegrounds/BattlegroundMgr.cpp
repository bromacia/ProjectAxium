...
void BattlegroundMgr::BuildBattlegroundListPacket(WorldPacket* data, uint64 guid, Player* player, BattlegroundTypeId bgTypeId, uint8 fromWhere)
{
    ...
    winner_kills = Axium::Honor::hk_honor_at_level(player->getLevel(), float(winner_kills));
    loser_kills = Axium::Honor::hk_honor_at_level(player->getLevel(), float(loser_kills));
