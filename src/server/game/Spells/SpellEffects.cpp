...
void Spell::SendLoot(uint64 guid, LootType loottype)
{
    ...
    if (gameObjTarget)
    {
        ...
        if (!gameObjTarget->isSpawned() && !player->HasGameMasterTagOn())
        {
...
void Spell::EffectAddHonor(SpellEffIndex /*effIndex*/)
{
    ...
    if (damage <= 50)
    {
        uint32 honor_reward = Axium::Honor::hk_honor_at_level(unitTarget->getLevel(), float(damage));
...
void Spell::EffectWeaponDmg(SpellEffIndex effIndex)
{
    ...
    uint32 damage = m_caster->MeleeDamageBonusDone(unitTarget, eff_damage, m_attackType, m_spellInfo, true);
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 48025:                                     // Headless Horseman's Mount
                {
                    ...
                    if (!pArea || (canFly && pArea->IsNoFlyZone()))
                        canFly = false;
