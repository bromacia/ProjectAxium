...
WorldObject* Spell::FindCorpseUsing()
{
    ...
    CellCoord p(Axium::ComputeCellCoord(m_caster->GetPositionX(), m_caster->GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();
    ...
    Axium::WorldObjectSearcher<T> searcher(m_caster, result, u_check);

    TypeContainerVisitor<Axium::WorldObjectSearcher<T>, GridTypeMapContainer > grid_searcher(searcher);
    cell.Visit(p, grid_searcher, *m_caster->GetMap(), *m_caster, max_range);

    if (!result)
    {
        TypeContainerVisitor<Axium::WorldObjectSearcher<T>, WorldTypeMapContainer > world_searcher(searcher);
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
        ...
        case 55342: // Mirror Image
            {
                ...
                UnitList targets;
                Axium::AnyUnfriendlyUnitInObjectRangeCheck u_check(unit, unit, unit->GetMap()->GetVisibilityRange());
                Axium::UnitListSearcher<Axium::AnyUnfriendlyUnitInObjectRangeCheck> searcher(unit, targets, u_check);
    ...
    if (unit->GetEntry() == 5925 && ((RedirectedSpell || m_spellInfo->IsNegativeAuraSpell() ||
        m_spellInfo->HasEffect(SPELL_EFFECT_DISPEL)) && (m_spellInfo->HasTargetA(TARGET_UNIT_TARGET_ENEMY) || m_spellInfo->HasTargetA(TARGET_UNIT_TARGET_ANY))))
    {
    ...
    else if (m_spellInfo->IsDelaySpell())
        ...
        switch (m_spellInfo->DmgClass)
        {
            case SPELL_DAMAGE_CLASS_MAGIC:
                ...
                else
                {
                    ...
                    if ((m_spellInfo->Id != 44461 && m_spellInfo->Id != 55361 && m_spellInfo->Id != 55362)
                        && !(unitTarget->HasAura(31661) && unitTarget->HasAura(33041) && unitTarget->HasAura(33042)
                        && unitTarget->HasAura(33043) && unitTarget->HasAura(42949) && unitTarget->HasAura(42950)))
    ...
    else if (m_damage > 0)
    {
        ...
        if (canEffectTrigger && missInfo != SPELL_MISS_REFLECT)
        {
            caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim, procEx, damageInfo.damage, m_attackType, m_spellInfo, m_triggeredByAuraSpell, damageInfo.splitDamage);
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    if (m_caster != unit)
    {
        ...
        else if (m_caster->IsFriendlyTo(unit))
        {
            ...
            {
                m_caster->SetInCombatState(unit->GetCombatTime() > 0, unit);
...
void Spell::SearchChainTarget(std::list<Unit*> &TagUnitMap, float max_range, uint32 num, SpellTargets TargetType)
{
    ...
    while (num)
    {
        ...
        {
            tempUnitMap.sort(Axium::ObjectDistanceOrderPred(cur));
            next = tempUnitMap.begin();
...
void Spell::SearchAreaTarget(std::list<Unit*> &TagUnitMap, float radius, SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
    ...
    Axium::SpellNotifierCreatureAndPlayer notifier(m_caster, TagUnitMap, radius, type, TargetType, pos, entry, m_spellInfo);
...
void Spell::SearchGOAreaTarget(std::list<GameObject*> &TagGOMap, float radius, SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
    ...
    Axium::GameObjectInRangeCheck check(pos->m_positionX, pos->m_positionY, pos->m_positionZ, radius, entry);
    Axium::GameObjectListSearcher<Axium::GameObjectInRangeCheck> searcher(m_caster, TagGOMap, check);
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargets TargetType, SpellEffIndex effIndex)
{
    switch (TargetType)
    {
        ...
        {
            Unit* target = NULL;
            Axium::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, range);
            Axium::UnitLastSearcher<Axium::AnyUnfriendlyUnitInObjectRangeCheck> searcher(m_caster, target, u_check);
        ...
        {
            Unit* target = NULL;
            Axium::AnyFriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, range);
            Axium::UnitLastSearcher<Axium::AnyFriendlyUnitInObjectRangeCheck> searcher(m_caster, target, u_check);
            m_caster->VisitNearbyObject(range, searcher);
            return target;
        }
    }
    ...
    else if (pushType)
    {
            ...
            case SPELL_TARGETS_ENTRY:
            {
                ...
                else
                {
                    ...
                    switch (m_spellInfo->Id)
                    {
                        ...
                        {
                            if (WorldObject* result = FindCorpseUsing<Axium::RaiseDeadObjectCheck>())
                            {
                        ...
                        case 51328:
                                    ...
                                    && m_targets.GetUnitTarget()->GetDisplayId() == m_targets.GetUnitTarget()->GetNativeDisplayId()))))
                            {
                                ...
                                WorldObject* result = FindCorpseUsing<Axium::ExplodeCorpseObjectCheck>();
        ...
        if (!unitList.empty())
        {
            ...
            if (maxSize && power != -1)
            {
                if (Powers(power) == POWER_HEALTH)
                {
                    ...
                    {
                        unitList.sort(Axium::HealthPctOrderPred());
                ...
                else
                {
                    ...
                    if (unitList.size() > maxSize)
                    {
                        unitList.sort(Axium::PowerPctOrderPred((Powers)power));
                        unitList.resize(maxSize);
                    }
                }
            }
            ...
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                Axium::RandomResizeList(unitList, maxTargets);
            }
        ...
        if (!gobjectList.empty())
        {
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                Axium::RandomResizeList(gobjectList, maxTargets);
            }
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER && !(m_spellInfo->Attributes & SPELL_ATTR0_PASSIVE))
    {
        ...
        if (m_caster->ToPlayer()->HasSpellCooldown(m_spellInfo->Id))
        {
            ...
            else if (m_spellInfo->Id != 15473 && m_spellInfo->Id != 46584)
    ...
    // zone check
    if (m_caster->GetTypeId() == TYPEID_UNIT || !m_caster->ToPlayer()->HasGameMasterTagOn())
    {
        uint32 zone, area;
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CHARGE:
            {
                ...
                if (GetSpellInfo()->NeedsExplicitUnitTarget())
                {
                    ...
                    {
                        m_caster->m_preGeneratedPath->SetPathLengthLimit(m_spellInfo->GetMaxRange(true) * 2.0f);
                        bool result = m_caster->m_preGeneratedPath->Calculate(pos.m_positionX, pos.m_positionY, pos.m_positionZ);
                        if (m_caster->m_preGeneratedPath->GetPathType() & PATHFIND_SHORT)
                            return SPELL_FAILED_OUT_OF_RANGE;
                        else if (!result && !m_caster->isPet() || m_caster->m_preGeneratedPath->GetPathType() & PATHFIND_NOPATH)
    ...
    if (Player* player = m_caster->ToPlayer())
    {
        ...
        if (Pet* pet = player->GetPet())
        {
            ...
            if (m_spellInfo->AttributesEx2 & SPELL_ATTR2_HEALTH_FUNNEL)
                if (!player->IsWithinLOSInMap(pet))
                    return SPELL_FAILED_LINE_OF_SIGHT;
    ...
    if (m_spellInfo->Id == 32182 || m_spellInfo->Id == 2825)
        if (m_caster->HasAura(57723) || m_caster->HasAura(57724))
            return SPELL_FAILED_CANT_DO_THAT_RIGHT_NOW;
    ...
    if (m_spellInfo->HasEffect(SPELL_EFFECT_LEAP) || m_spellInfo->HasEffect(SPELL_EFFECT_CHARGE) ||
        ...
        if (m_caster->HasAura(32727) || m_caster->HasAura(44521))
            return SPELL_FAILED_CANT_DO_THAT_RIGHT_NOW;
    ...
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN && m_spellInfo->SpellFamilyFlags[0] == 0x8000000)
        if (Creature* fireTotem = m_caster->GetMap()->GetCreature(m_caster->ToPlayer()->m_SummonSlot[1])->ToTotem())
    ...
    if (m_spellInfo->Id != 19574 && m_spellInfo->Id != 34471) // Bestial Wrath, Beast Within
    {
        if (unitflag & UNIT_FLAG_STUNNED)
        {
            ...
            if (usableInStun)
            {
                ...
                {
                    if ((*i)->GetSpellInfo()->GetAllEffectsMechanicMask() && !((*i)->GetSpellInfo()->GetAllEffectsMechanicMask() & ((1<<MECHANIC_STUN)|(1<<MECHANIC_FREEZE)))) // Freeze mechanics should be considered stuns aswell
                    {
                        foundNotStun = true;
                        break;
                    }
...
SpellCastResult Spell::CheckItems()
{
    ...
    if (m_spellInfo->RequiresSpellFocus)
    {
        ...
        GameObject* ok = NULL;
        Axium::GameObjectFocusCheck go_check(m_caster, m_spellInfo->RequiresSpellFocus);
        Axium::GameObjectSearcher<Axium::GameObjectFocusCheck> checker(m_caster, ok, go_check);

        TypeContainerVisitor<Axium::GameObjectSearcher<Axium::GameObjectFocusCheck>, GridTypeMapContainer > object_checker(checker);
...
void Spell::SelectTrajTargets()
{
    ...
    unitList.sort(Axium::ObjectDistanceOrderPred(m_caster));
...
void Spell::TriggerGlobalCooldown()
{
    ...
    if (m_spellInfo->StartRecoveryTime >= MIN_GCD && m_spellInfo->StartRecoveryTime <= MAX_GCD)
    {
        ...
        // Apply haste rating
        gcd = int32(float(gcd) * m_caster->m_positiveCastTimePrecent); // only reducing mods
        if (gcd < MIN_GCD)
            gcd = MIN_GCD;
        else if (gcd > MAX_GCD)
            gcd = MAX_GCD;
