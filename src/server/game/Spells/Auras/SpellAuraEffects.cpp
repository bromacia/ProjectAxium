...
// such checks will speedup axium change amount/send for client operations
...
void AuraEffect::HandleModInvisibility(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        UnitList targets;
        Axium::AnyUnfriendlyUnitInObjectRangeCheck u_check(target, target, target->GetMap()->GetVisibilityRange());
        Axium::UnitListSearcher<Axium::AnyUnfriendlyUnitInObjectRangeCheck> searcher(target, targets, u_check);
...
void AuraEffect::HandleModStateImmunityMask(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    // Bladestorm, Killing Spree immunities
...
void AuraEffect::HandleModMechanicImmunity(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    switch (GetId())
    {
        case 19574: // Beastial Wrath
        case 34471: // Beast Within
            mechanic = (1 << MECHANIC_SNARE) | (1 << MECHANIC_ROOT) | (1 << MECHANIC_FEAR) | (1 << MECHANIC_STUN) | \
                (1 << MECHANIC_SLEEP) | (1 << MECHANIC_CHARM) | (1 << MECHANIC_SAPPED) | (1 << MECHANIC_HORROR) | \
                (1 << MECHANIC_POLYMORPH) | (1 << MECHANIC_DISORIENTED) | (1 << MECHANIC_FREEZE) | (1 << MECHANIC_TURN) | \
                (1 << MECHANIC_BANISH) | (1 << MECHANIC_BANISH) | (1 << MECHANIC_KNOCKOUT);
            target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_SNARE, apply);
            target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_ROOT, apply);
            target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_FEAR, apply);
            target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_STUN, apply);
            ...
            target->ApplySpellImmune(GetId(), IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK_DEST, apply);
            break;
        case 64364: // Aura Mastery - Concentration Aura
                ...
                case SPELLFAMILY_GENERIC:
                    switch (GetId())
                    {
                        case 2584: // Waiting to Resurrect
                            ...
                            break;
                        case 43681: // Inactive (Needs implemented)
                        {
                ...
                case SPELLFAMILY_DRUID:
                    ...
                    if (GetSpellInfo()->SpellFamilyFlags[1] & 0x10)
                    {
                        ...
                        if (caster)
                        {
                            ...
                            if (caster->HasAura(33786) || caster->HasAura(710) || caster->HasAura(18647))
                                return;
            ...
            switch (GetSpellInfo()->Id)
            {
                ...
                case 51690:
                {
                    ...
                    UnitList targets;
                    {
                        ...
                        CellCoord p(Axium::ComputeCellCoord(target->GetPositionX(), target->GetPositionY()));
                        Cell cell(p);

                        Axium::KillingSpreeObjectRangeCheck u_check(target, radius);
                        Axium::UnitListSearcher<Axium::KillingSpreeObjectRangeCheck> checker(target, targets, u_check);

                        TypeContainerVisitor<Axium::UnitListSearcher<Axium::KillingSpreeObjectRangeCheck>, GridTypeMapContainer > grid_object_checker(checker);
                        TypeContainerVisitor<Axium::UnitListSearcher<Axium::KillingSpreeObjectRangeCheck>, WorldTypeMapContainer > world_object_checker(checker);
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    caster->CalcAbsorbResist(target, GetSpellInfo()->GetSchoolMask(), DOT, damage, &absorb, &resist, GetSpellInfo(), -1, &splitDamage);
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    caster->CalcAbsorbResist(target, GetSpellInfo()->GetSchoolMask(), DOT, damage, &absorb, &resist, m_spellInfo, -1, &splitDamage);
