...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
    ...
    CellCoord p = Axium::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!p.IsCoordValid())
    {
...
{
    CellCoord cellCoord = Axium::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
bool Map::AddToMap(T *obj)
{
    ...
    CellCoord cellCoord = Axium::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<Axium::ObjectUpdater, GridTypeMapContainer> &gridVisitor, TypeContainerVisitor<Axium::ObjectUpdater, WorldTypeMapContainer> &worldVisitor)
{
...
void Map::Update(const uint32 t_diff)
{
    ...
    Axium::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<Axium::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<Axium::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
    ...
    for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); ++i)
    {
        ...
        for (uint32 x = cell_min.x_coord; x < cell_max.x_coord; ++x)
        {
            for (uint32 y = cell_min.y_coord; y < cell_max.y_coord; ++y)
            {
                ...
                Axium::DelayedUnitRelocation cell_relocation(cell, pair, *this, MAX_VISIBILITY_DISTANCE);
                TypeContainerVisitor<Axium::DelayedUnitRelocation, GridTypeMapContainer  > grid_object_relocation(cell_relocation);
                TypeContainerVisitor<Axium::DelayedUnitRelocation, WorldTypeMapContainer > world_object_relocation(cell_relocation);
...
{
    cell.SetNoCreate();
    Axium::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<Axium::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
...
{
    Axium::VisibleNotifier notifier(*player);

    cell.SetNoCreate();
    TypeContainerVisitor<Axium::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<Axium::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
uint32 Map::GetPlayersCountExceptGMs() const
{
    ...
    for (MapRefManager::const_iterator itr = m_mapRefManager.begin(); itr != m_mapRefManager.end(); ++itr)
        if (!itr->getSource()->HasGameMasterTagOn())
            ++count;
    return count;
}
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        {
            GridCoord p2 = Axium::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
...
bool InstanceMap::CanEnter(Player* player)
{
    ...
    if (!playerList.isEmpty())
        for (PlayerList::const_iterator i = playerList.begin(); i != playerList.end(); ++i)
            ...
            {
                if (iPlayer->HasGameMasterTagOn()) // bypass GMs
                    continue;
