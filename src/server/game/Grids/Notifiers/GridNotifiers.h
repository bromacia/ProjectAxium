...
namespace Axium
{
    ...
    class KillingSpreeObjectRangeCheck
    {
        public:
            KillingSpreeObjectRangeCheck(Unit const* funit, float range)
                : i_funit(funit), i_range(range) {}
            ...
            bool operator()(const Unit* u)
            {
                return u->isAlive()
                    ...
                    && i_funit->canSeeOrDetect(u)
                    && !u->isTotem()
                    && !u->HasAuraType(SPELL_AURA_MOD_STEALTH)
                    && !u->HasAuraType(SPELL_AURA_MOD_INVISIBILITY);
            }
        private:
            Unit const* i_funit;
            float i_range;
    ...
    class NearestAttackableUnitInTrapRangeCheck
    {
        public:
            NearestAttackableUnitInTrapRangeCheck(WorldObject const* obj, Unit const* funit, float range) : i_obj(obj), i_funit(funit), i_range(range) {}
            bool operator()(Unit* u)
            {
                if (u->isTargetableForAttack() && i_obj->IsWithinDistInMap(u, i_range) && !i_funit->IsFriendlyTo(u))
                {
                    i_range = i_obj->GetDistance(u);        // use found unit range as new range limit for next check
                    return true;
                }
        ...
        private:
            ...
            // prevent clone this object
            NearestAttackableUnitInTrapRangeCheck(NearestAttackableUnitInTrapRangeCheck const&);
    };
    ...
    */
    ...
    public:
        ...
        bool operator() (Player* player)
        {
            ...
            if (!player->HasGameMasterTagOn() && player->isAlive() && !unit->IsWithinDist(player, fRange, false))
                return true;
