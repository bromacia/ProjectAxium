    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_FLEE_FOR_ASSIST:
        {
            ...
            if (e.action.flee.withEmote)
                sCreatureTextMgr->SendChatString(me, sObjectMgr->GetAxiumStringForDBCLocale(LANG_FLEE), CHAT_MSG_MONSTER_EMOTE);
...
ObjectList* SmartScript::GetWorldObjectsInDist(float dist)
{
    ...
    if (obj)
    {
        Axium::AllWorldObjectsInRange u_check(obj, dist);
        Axium::WorldObjectListSearcher<Axium::AllWorldObjectsInRange> searcher(obj, *targets, u_check);
...
Unit* SmartScript::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff)
{
    ...
    CellCoord p(Axium::ComputeCellCoord(me->GetPositionX(), me->GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();

    Unit* unit = NULL;

    Axium::MostHPMissingInRange u_check(me, range, MinHPDiff);
    Axium::UnitLastSearcher<Axium::MostHPMissingInRange> searcher(me, unit, u_check);

    TypeContainerVisitor<Axium::UnitLastSearcher<Axium::MostHPMissingInRange>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
void SmartScript::DoFindFriendlyCC(std::list<Creature*>& _list, float range)
{
    ...
    Axium::FriendlyCCedInRange u_check(me, range);
    Axium::CreatureListSearcher<Axium::FriendlyCCedInRange> searcher(me, _list, u_check);

    TypeContainerVisitor<Axium::CreatureListSearcher<Axium::FriendlyCCedInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
void SmartScript::DoFindFriendlyMissingBuff(std::list<Creature*>& list, float range, uint32 spellid)
{
    ...
    Axium::FriendlyMissingBuffInRange u_check(me, range, spellid);
    Axium::CreatureListSearcher<Axium::FriendlyMissingBuffInRange> searcher(me, list, u_check);

    TypeContainerVisitor<Axium::CreatureListSearcher<Axium::FriendlyMissingBuffInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
