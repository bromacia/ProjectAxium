        ...
        for (GroupReference* itr = _group->GetFirstMember(); itr != NULL; itr = itr->next())
            if (Player* member = itr->getSource())
                if (member->isAlive() && member->IsAtGroupRewardDistance(_victim))
                {
                    ...
                    // 2.4. _maxNotGrayMember - maximum level of alive group member within reward distance,
                    //      for whom victim is not gray;
                    uint32 grayLevel = Axium::XP::GetGrayLevel(lvl);
...
inline void KillRewarder::_InitXP(Player* player)
{
    ...
    if (_isBattleGround || (!_isPvP && !_killer->GetVehicle()))
        _xp = Axium::XP::Gain(player, _victim);
}
    ...
    if (_maxLevel)
    {
        ...
        if (!_isBattleGround || _xp)
        {
            ...
            if (!_isBattleGround)
            {
                ...
                _groupRate = Axium::XP::xp_in_group_rate(_count, isRaid);
            }
...
bool Player::Create(uint32 guidlow, CharacterCreateInfo* createInfo)
{
    ...
    setOriginalRace();
    m_team = TeamForRace(getOriginalRace());
    SetFakeRace();
    setFactionForRace(getOriginalRace());
...
uint32 Player::EnvironmentalDamage(EnviromentalDamage type, uint32 damage)
{
    ...
    if (type == DAMAGE_LAVA)
        CalcAbsorbResist(this, SPELL_SCHOOL_MASK_FIRE, DIRECT_DAMAGE, damage, &absorb, &resist, NULL, -1, &splitDamage);
    else if (type == DAMAGE_SLIME)
        CalcAbsorbResist(this, SPELL_SCHOOL_MASK_NATURE, DIRECT_DAMAGE, damage, &absorb, &resist, NULL, -1, &splitDamage);
...
void Player::Update(uint32 p_time)
{
    ...
    if (HasUnitState(UNIT_STATE_MELEE_ATTACKING) && !HasUnitState(UNIT_STATE_CASTING))
    {
        if (Unit* pVictim = getVictim())
        {
            ...
            {
                if (!IsWithinObjectSizeDistance(pVictim, MELEE_RANGE))
                {
        ...
        //if (!map || map->CanEnter(this))
        {
            ...
            if (mEntry->IsBattleArena())
            {
                ...
                if (pet)
                {
                    ...
                    pet->RemoveAllPetSpellCooldowns(this);
                }
            }
...
void Player::Regenerate(Powers power)
{
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            ...
            if (recentCast) // Axium Updates Mana in intervals of 2s, which is correct
...
void Player::SendLogXPGain(uint32 GivenXP, Unit* victim, uint32 BonusXP/*, float group_rate*/)
{
    ...
    data << uint8(0);                  // does the GivenXP include a RaF bonus?
    GetSession()->SendPacket(&data);
}
...
void Player::GiveLevel(uint8 level)
{
    ...
    sObjectMgr->GetPlayerLevelInfo(getOriginalRace(), getClass(), level, &info);
...
void Player::InitStatsForLevel(bool reapplyMods)
{
    ...
    PlayerLevelInfo info;
    sObjectMgr->GetPlayerLevelInfo(getOriginalRace(), getClass(), getLevel(), &info);
...
bool Player::addSpell(uint32 spellId, bool active, bool learning, bool dependent, bool disabled, bool loading, bool singleSpell)
{
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId);
    if (!spellInfo)
    {
...
void Player::DeleteFromDB(uint64 playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    // if we want to finally delete the character
            ...
            if (CanJoinConstantChannelInZone(channel, current_zone))
            {
                if (!(channel->flags & CHANNEL_DBC_FLAG_GLOBAL))
                {
                    ...
                    if (channel->flags & CHANNEL_DBC_FLAG_CITY_ONLY)
                        currentNameExt = sObjectMgr->GetAxiumStringForDBCLocale(LANG_CHANNEL_CITY);
                    else
...
void Player::UpdateCombatSkills(Unit* victim, WeaponAttackType attType, bool defence)
{
    ...
    uint8 greylevel = Axium::XP::GetGrayLevel(plevel);
...
void Player::SendActionButtons(uint32 state) const
{
    ...
    /*
        state can be 0, 1, 2
        0 - Looks to be sent when initial action buttons get sent, however on Axium we use 1 since 0 had some difficulties
        1 - Used in any SMSG_ACTION_BUTTONS packet with button data on Axium. Only used after spec swaps on retail.
...
void Player::SendMessageToSetInRange(WorldPacket* data, float dist, bool self)
{
    ...
    Axium::MessageDistDeliverer notifier(this, data, dist);
...
void Player::SendMessageToSetInRange(WorldPacket* data, float dist, bool self, bool own_team_only)
{
    ...
    Axium::MessageDistDeliverer notifier(this, data, dist, own_team_only);
    VisitNearbyWorldObject(dist, notifier);
}
...
void Player::SendMessageToSet(WorldPacket* data, Player const* skipped_rcvr)
{
    ...
    Axium::MessageDistDeliverer notifier(this, data, GetVisibilityRange(), false, skipped_rcvr);
...
int32 Player::CalculateReputationGain(uint32 creatureOrQuestLevel, int32 rep, int32 faction, bool for_quest, bool noQuestBonus)
{
    ...
    if (rate != 1.0f && creatureOrQuestLevel <= Axium::XP::GetGrayLevel(getLevel()))
        percent *= rate;
...
void Player::RewardReputation(Unit* pVictim, float rate)
{
    ...
    ReputationOnKillEntry const* Rep = sObjectMgr->GetReputationOnKillEntry(pVictim->ToCreature()->GetCreatureInfo()->Entry);
    if (!Rep)
        return;
    ...
    if (repfaction1 && (!Rep->team_dependent || team == ALLIANCE))
    {
        int32 donerep1 = CalculateReputationGain(pVictim->getLevel(), Rep->repvalue1, ChampioningFaction ? ChampioningFaction : repfaction1, false);
    ...
    if (repfaction2 && (!Rep->team_dependent || team == HORDE))
    {
        int32 donerep2 = CalculateReputationGain(pVictim->getLevel(), Rep->repvalue2, ChampioningFaction ? ChampioningFaction : repfaction2, false);
...
bool Player::RewardHonor(Unit* uVictim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    if (honor_f <= 0)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            uint8 k_grey = Axium::XP::GetGrayLevel(k_level);
            ...
            honor_f = ceil(Axium::Honor::hk_honor_at_level_f(k_level) * (v_level - k_grey) / (k_level - k_grey));
...
void Player::ModifyArenaPoints(int32 value, SQLTransaction* trans /*=NULL*/)
{
    ...
    if (trans && !trans->null())
        (*trans)->PAppend("UPDATE characters SET totalArenaPoints=%u WHERE guid=%u", newValue, GetGUIDLow());
}
...
InventoryResult Player::CanEquipItem(uint8 slot, uint16 &dest, Item* pItem, bool swap, bool not_loading) const
{
    ...
    if (pItem)
    {
        ...
        if (pProto)
        {
            ...
            {
                // Do not allow polearms to be equipped in the offhand slot (rare case for the only 1h polearm 41750)
...
bool Player::CanCompleteQuest(uint32 quest_id)
{
    if (quest_id)
    {
        ...
        {
            if (qInfo->HasFlag(QUEST_AXIUM_FLAGS_DELIVER))
            {
                for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
                {
            ...
            if (qInfo->HasFlag(QUEST_AXIUM_FLAGS_KILL_OR_CAST | QUEST_AXIUM_FLAGS_SPEAKTO))
            {
            ...
            if (qInfo->HasFlag(QUEST_AXIUM_FLAGS_PLAYER_KILL))
                if (qInfo->GetPlayersSlain() != 0 && q_status.m_playercount < qInfo->GetPlayersSlain())
                    return false;

            if (qInfo->HasFlag(QUEST_AXIUM_FLAGS_EXPLORATION_OR_EVENT) && !q_status.m_explored)
                return false;

            if (qInfo->HasFlag(QUEST_AXIUM_FLAGS_TIMED) && q_status.m_timer == 0)
                return false;
...
bool Player::CanCompleteRepeatableQuest(Quest const* quest)
{
    ...
    if (quest->HasFlag(QUEST_AXIUM_FLAGS_DELIVER))
        for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
...
void Player::AddQuest(Quest const* quest, Object* questGiver)
{
    ...
    if (quest->HasFlag(QUEST_AXIUM_FLAGS_KILL_OR_CAST | QUEST_AXIUM_FLAGS_SPEAKTO))
    {
        for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
            questStatusData.m_creatureOrGOcount[i] = 0;
    }

    if (quest->HasFlag(QUEST_AXIUM_FLAGS_PLAYER_KILL))
    ...
    uint32 qtime = 0;
    if (quest->HasFlag(QUEST_AXIUM_FLAGS_TIMED))
    {
...
{
    if (!m_timedquests.empty() && qInfo->HasFlag(QUEST_AXIUM_FLAGS_TIMED))
    {
        if (msg)
...
// not used in Axium, but used in scripting code
...
//not used in Axiumd, function for external script library
...
void Player::ItemAddedQuestCheck(uint32 entry, uint32 count)
{
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        if (!qInfo || !qInfo->HasFlag(QUEST_AXIUM_FLAGS_DELIVER))
            continue;
...
void Player::KilledMonsterCredit(uint32 entry, uint64 guid)
{
    ...
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        {
            if (qInfo->HasFlag(QUEST_AXIUM_FLAGS_KILL_OR_CAST))
            {
                for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
                {
...
void Player::SendQuestReward(Quest const* quest, uint32 XP, Object* questGiver)
{
    ...
    data << 10 * Axium::Honor::hk_honor_at_level(getLevel(), quest->GetRewHonorMultiplier());
...
bool Player::LoadFromDB(uint32 guid, SQLQueryHolder *holder)
{
    ...
    _LoadIntoDataField(fields[66].GetCString(), PLAYER_FIELD_KNOWN_TITLES, KNOWN_TITLES_SIZE*2);
    ...
    SetUInt32Value(PLAYER_BYTES_3, (fields[51].GetUInt16() & 0xFFFE) | fields[5].GetUInt8());
    SetUInt32Value(PLAYER_FLAGS, fields[11].GetUInt32());
    SetInt32Value(PLAYER_FIELD_WATCHED_FACTION_INDEX, fields[50].GetUInt32());

    SetUInt64Value(PLAYER_FIELD_KNOWN_CURRENCIES, fields[49].GetUInt64());

    SetUInt32Value(PLAYER_AMMO_ID, fields[65].GetUInt32());
    ...
    uint32 instanceId = fields[60].GetUInt8();
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    m_weeklyArenaPoints = fields[39].GetUInt32();
    SetArenaPoints(fields[40].GetUInt32());
    m_arenaPointsCap = fields[41].GetUInt32();
    ...
    SetHonorPoints(fields[42].GetUInt32());
    ...
    for (uint8 i = 0; i < MAX_POWERS; ++i)
    {
        uint32 savedPower = fields[53+i].GetUInt32();
...
void Player::_LoadActions(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                sLog->outDetail("  ...at loading, and will deleted in DB also");
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timeDiff)
{
    ...
    if (result)
    {
        ...
        {
            std::string subject = GetSession()->GetAxiumString(LANG_NOT_EQUIPPED_ITEM);
    ...
    if (result)
    {
        do
        {
            ...
            if (quest)
            {
                ...
                if (quest->HasFlag(QUEST_AXIUM_FLAGS_TIMED) && !GetQuestRewardStatus(quest_id))
                {
                    AddTimedQuest(quest_id);
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!HasGameMasterTagOn() && ar)
    {
        ...
        {
            GetSession()->SendAreaTriggerMessage("%s", GetSession()->GetAxiumString(LANG_INSTANCE_CLOSED));
            return false;
        }
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetAxiumString(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, sObjectMgr->GetItemTemplate(missingItem)->Name1.c_str());
                else if (LevelMin)
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetAxiumString(LANG_LEVEL_MINREQUIRED), LevelMin);
            }
            return false;
        }
    }
...
void Player::SaveToDB(bool create /*=false*/)
{
    ...
    if (create)
    {
        ...
        stmt->setUInt8(index++, getOriginalRace());
        ...
        stmt->setString(index++, ss.str());
        stmt->setUInt32(index++, m_weeklyArenaPoints);
        stmt->setUInt32(index++, GetArenaPoints());
        stmt->setUInt32(index++, m_arenaPointsCap);
...
void Player::UpdateVisibilityForPlayer()
{
    ...
    Axium::VisibleNotifier notifier(*this);
...
void Player::learnDefaultSpells()
{
    ...
    {
        uint32 tspell = *itr;
        sLog->outDebug(LOG_FILTER_PLAYER_LOADING, "PLAYER (Class: %u Race: %u): Adding initial spell, id = %u", uint32(getClass()), uint32(getOriginalRace()), tspell);
...
{
    uint8 v_level = pVictim->getLevel();
    uint8 k_grey  = Axium::XP::GetGrayLevel(getLevel());
...
{
    return (!isTotalImmune() &&                             // not totally immune
...
bool Player::CheckExtendedCost2(const ItemTemplate* vItemTemplate)
{
    ...
    if (extendedCost2.Required_2v2_Rating)
        if (sPvPMgr->GetLifetime2v2Rating(GetGUID()) < extendedCost2.Required_2v2_Rating)
            return false;

    if (extendedCost2.Required_3v3_Rating)
        if (sPvPMgr->GetLifetime3v3Rating(GetGUID()) < extendedCost2.Required_3v3_Rating)
            return false;

    if (extendedCost2.Required_5v5_Rating)
        if (sPvPMgr->GetLifetime5v5Rating(GetGUID()) < extendedCost2.Required_5v5_Rating)
            return false;
