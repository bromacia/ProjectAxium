...
bool Unit::SetPosition(float x, float y, float z, float orientation, bool teleport)
{
    ...
    if (!Axium::IsValidMapCoord(x, y, z, orientation))
        return false;
...
bool Unit::IsWithinObjectSizeDistance(const Unit* obj, float dist) const
{
    if (!obj || !IsInMap(obj) || !InSamePhase(obj))
        return false;
...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage* damageInfo, int32 damage, SpellInfo const* spellInfo, WeaponAttackType attackType, bool crit, int32 calc_resist)
{
    ...
    if (damage > 0)
    {
        CalcAbsorbResist(victim, damageSchoolMask, SPELL_DIRECT_DAMAGE, damage, &damageInfo->absorb, &damageInfo->resist, spellInfo, calc_resist, &damageInfo->splitDamage);
...
uint32 Unit::CalcArmorReducedDamage(Unit* victim, const uint32 damage, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/)
{
    ...
    //todo: srsly fix pene armor in the proper way....
...
uint32 Unit::CalcSpellResistance(Unit* victim, SpellSchoolMask schoolMask, bool binary, SpellInfo const* spellInfo) const
{
    // Magic damage, check for resists
    ...
    if (spellInfo && spellInfo->AttributesCu & SPELL_ATTR0_CU_BINARY)
    {
    ...
    for (; resistance < 10, r >= probabilitySum; ++resistance)
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32* absorb, uint32* resist, SpellInfo const* spellInfo, int32 calc_resist, bool* splitDamage)
{
    ...
    vSchoolAbsorbCopy.sort(Axium::AbsorbAuraOrderPred());
    ...
    if (victim != this)
    {
        ...
        for (AuraEffectList::iterator itr = vSplitDamagePctCopy.begin(), next; (itr != vSplitDamagePctCopy.end()) &&  (dmgInfo.GetDamage() > 0); ++itr)
        {
            ...
            if (caster->HasAura(642))
                caster->SendSpellDamageImmune(caster, (*itr)->GetSpellInfo()->Id);
            else
            {
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst (const Unit* victim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance) const
{
    ...
    else
    {
        ...
        else if (HasUnitTypeMask(UNIT_MASK_GUARDIAN))
            dodge_chance -= ((Guardian*)this)->GetModExpertise() * 25;
        else
    ...
    else
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
            ...
            parry_chance -= ((Guardian*)this)->GetModExpertise() * 25;
        else
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    // Prevent spells that require behind target from dodging, parrying and blocking
    ...
    if (canDodge)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
            ...
            dodgeChance -= ((Guardian*)this)->GetModExpertise() * 25;
        else
    ...
    if (canParry)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
            ...
            parryChance -= ((Guardian*)this)->GetModExpertise() * 25;
        else
...
uint32 Unit::CalcMagicSpellHitChance(Unit* victim, SpellSchoolMask schoolMask, SpellInfo const* spellInfo)
{
    ...
    if (source && !isTotem())
        source->ToPlayer()->ApplySpellMod(spellInfo->Id, SPELLMOD_RESIST_MISS_CHANCE, modHitChance);
    else
        source = this;
    ...
    if (!(spellInfo->AttributesEx3 & SPELL_ATTR3_IGNORE_HIT_RESULT))
    {
        ...
        if (!(spellInfo->SpellFamilyName == SPELLFAMILY_WARLOCK && spellInfo->SpellIconID == 3178)) // Chaos Bolt should ignore it
...
void Unit::RemoveAllNegativeAuras()
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        ...
        if ((!(aura->GetSpellInfo()->AttributesEx4 & SPELL_ATTR4_UNK21) && !aura->IsPassive()
            && (!aurApp->IsPositive() || !(aura->GetSpellInfo()->AttributesEx3 & SPELL_ATTR3_DEATH_PERSISTENT)))
            && !aurApp->IsPositive() && aura->GetSpellInfo()->Id != 7267 && aura->GetSpellInfo()->Id != 26013)
            RemoveAura(iter);
        else
            ++iter;
    }
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 63280: // Glyph of Totem of Wrath
                {
                    ...
                    if (Creature* totem = GetMap()->GetCreature(m_SummonSlot[1]))
                    {
        ...
        case 48090:
        {
            ...
            if (isPet())
                if (Unit* owner = GetOwner())
                {
                    ...
                    {
                        basepoints0 = int32((aurEff->GetAmount() * owner->SpellBaseDamageBonusDone(SpellSchoolMask(SPELL_SCHOOL_MASK_MAGIC)) + 100.0f) / 100.0f); // TODO: Is it right?ght?
...
bool Unit::Attack(Unit* victim, bool meleeAttack)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER)
    {
        if (victim->ToPlayer()->HasGameMasterTagOn())
            return false;
    }
    else
    {
...
void Unit::RemoveAllTempSummons()
{
    ...
    for (ControlList::iterator itr = m_Controlled.begin(), next; itr != m_Controlled.end(); itr = next)
    {
        next = itr; ++next;
    ...
    if (uint32 aura = spellInfo->Effects[index].ApplyAuraName)
    {
        ...
        for (AuraEffectList::const_iterator iter = immuneAuraApply.begin(); iter != immuneAuraApply.end(); ++iter)
            if ((spellInfo->Dispel == DISPEL_MAGIC || spellInfo->Dispel == DISPEL_CURSE
               || spellInfo->Dispel == DISPEL_DISEASE || spellInfo->Dispel == DISPEL_POISON) // Magic, Curse, Disease, or Poison Debuffs
...
uint32 Unit::MeleeDamageBonusDone(Unit* victim, uint32 pdamage, WeaponAttackType attType, const SpellInfo* spellProto, bool skipPctMods)
{
    if (!victim || pdamage == 0)
        return 0;
...
uint32 Unit::MeleeDamageBonusTaken(Unit* attacker, uint32 pdamage, WeaponAttackType attType, const SpellInfo* spellProto)
{
    if (pdamage == 0)
        return 0;
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
    ...
    if (creatureAttacker && creatureAttacker->GetCreatureInfo()->type_flags & CREATURE_TYPEFLAGS_PARTY_MEMBER)
        return false;
...
bool Unit::_IsValidAssistTarget(Unit const* target, SpellInfo const* bySpell) const
{
    ...
    if (GetReactionTo(target) <= REP_NEUTRAL
        && target->GetReactionTo(this) <= REP_NEUTRAL
        && (!ToCreature() || !(ToCreature()->GetCreatureInfo()->type_flags & CREATURE_TYPEFLAGS_PARTY_MEMBER)))
        return false;
        ...
        && !((target->GetByteValue(UNIT_FIELD_BYTES_2, 1) & UNIT_BYTE2_FLAG_PVP)))
    {
        if (Creature const* creatureTarget = target->ToCreature())
            return creatureTarget->GetCreatureInfo()->type_flags & CREATURE_TYPEFLAGS_PARTY_MEMBER || creatureTarget->GetCreatureInfo()->type_flags & CREATURE_TYPEFLAGS_AID_PLAYERS;
    }
    return true;
}
...
void Unit::TauntApply(Unit* taunter)
{
    ...
    if (!taunter || (taunter->GetTypeId() == TYPEID_PLAYER && taunter->ToPlayer()->HasGameMasterTagOn()))
        return;
...
void Unit::ModSpellCastTime(SpellInfo const* spellProto, int32 & castTime, Spell* spell)
{
    ...
    if (spellProto->HasEffect(SPELL_EFFECT_TRANS_DOOR)) // Don't allow gameobject spawning to be instant casts (mage table, soulwell, ect)
        return;
...
uint32 Unit::GetCreatePowers(Powers power) const
{
    ...
    switch (power)
    {
        ...
        case POWER_HAPPINESS: return (GetTypeId() == TYPEID_PLAYER || !((Creature const*)this)->isPet() || ((Pet const*)this)->getPetType() != HUNTER_PET ? 0 : MAX_HAPPINESS);
...
Unit* Unit::SelectNearbyTarget(Unit* exclude, float dist) const
{
    ...
    Axium::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, this, dist);
    Axium::UnitListSearcher<Axium::AnyUnfriendlyUnitInObjectRangeCheck> searcher(this, targets, u_check);
...
bool Unit::IsInPartyWith(Unit const* unit) const
{
    ...
    else if ((u2->GetTypeId() == TYPEID_PLAYER && u1->GetTypeId() == TYPEID_UNIT && u1->ToCreature()->GetCreatureInfo()->type_flags & CREATURE_TYPEFLAGS_PARTY_MEMBER) ||
        (u1->GetTypeId() == TYPEID_PLAYER && u2->GetTypeId() == TYPEID_UNIT && u2->ToCreature()->GetCreatureInfo()->type_flags & CREATURE_TYPEFLAGS_PARTY_MEMBER))
        return true;
    else
        return false;
}
...
void Unit::UpdateObjectVisibility(bool forced)
{
    ...
    else
    {
        ...
        Axium::AIRelocationNotifier notifier(*this);
