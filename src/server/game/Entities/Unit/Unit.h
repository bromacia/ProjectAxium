...
struct SpellNonMeleeDamage{
    SpellNonMeleeDamage(Unit* _attacker, Unit* _target, uint32 _SpellID, uint32 _schoolMask)
        ...
        absorb(0), resist(0), physicalLog(false), unused(false), blocked(0), HitInfo(0), cleanDamage(0), splitDamage(false)
    {}
...
class Unit : public WorldObject
{
    public:
        ...
        bool IsWithinObjectSizeDistance(const Unit* obj, float dist) const;
        bool IsWithinExactDistance(const Unit* obj, float dist) const;
        ...
        uint32 getOriginalRaceMask() const { return 1 << (getRace(true) - 1); }
        ...
        // player or player's pet resilience (-1%)
        uint32 GetMeleeDamageReduction(uint32 damage) const { return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_MELEE, 2.0f, 33.0f, damage); }
        uint32 GetRangedDamageReduction(uint32 damage) const { return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_RANGED, 2.0f, 33.0f, damage); }
        uint32 GetSpellDamageReduction(uint32 damage) const { return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_SPELL, 2.0f, 33.0f, damage); }
        ...
        inline bool IsInDisallowedMountForm() const
        {
            ...
            return form != FORM_NONE && form != FORM_BATTLESTANCE && form != FORM_BERSERKERSTANCE && form != FORM_DEFENSIVESTANCE &&
                form != FORM_SHADOW && form != FORM_STEALTH && form != FORM_SHADOW_DANCE && form != FORM_UNDEAD;
        }
        ...
        uint32 MeleeDamageBonusDone(Unit *pVictim, uint32 damage, WeaponAttackType attType, const SpellInfo* spellProto = NULL, bool skipPctMods = false);
        uint32 MeleeDamageBonusTaken(Unit* attacker, uint32 pdamage,WeaponAttackType attType, const SpellInfo* spellProto = NULL);
