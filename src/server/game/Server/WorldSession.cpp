...
{
    sLog->outDetail("Client (account %u) send packet %s (%u) with size " SIZEFMTD " but expected %u (attempt to crash server?), skipped",
...
{
    sLog->outDetail("SESSION (account: %u, guidlow: %u, char: %s): received unexpected opcode %s (0x%.4X, status: %s) %s",
...
{
    sLog->outDetail("SESSION: opcode %s (0x%.4X) have unprocessed tail data (read stop at %u from %u)",
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
            ...
            _recvQueue.next(packet, updater))
    {
        ...
        {
            sLog->outDetail("SESSION: received non-existed opcode %s (0x%.4X)", LookupOpcodeName(packet->GetOpcode()), packet->GetOpcode());
        ...
        else
        {
            ...
            try
            {
                switch (opHandle.status)
                {
                    ...
                    case STATUS_AUTHED:
                        ...
                        break;
                    case STATUS_NEVER:
                        sLog->outDetail("SESSION (account: %u, guidlow: %u, char: %s): received not allowed opcode %s (0x%.4X)",
                            GetAccountId(), m_GUIDLow, _player ? _player->GetName() : "<none>",
            ...
            {
                sLog->outDetail("WorldSession::Update ByteBufferException occured while parsing a packet (opcode: %u) from client %s, accountid=%u. Skipped packet.",
...
void WorldSession::LogoutPlayer(bool Save)
{
    ...
    if (_player)
    {
        ...
        if (!_player->m_InstanceValid && !_player->HasGameMasterTagOn())
...
{
    sLog->outDetail("SESSION: received unhandled opcode %s (0x%.4X)", LookupOpcodeName(recvPacket.GetOpcode()), recvPacket.GetOpcode());
}

void WorldSession::Handle_EarlyProccess(WorldPacket& recvPacket)
{
    sLog->outDetail("SESSION: received opcode %s (0x%.4X) that must be processed in WorldSocket::OnRead", LookupOpcodeName(recvPacket.GetOpcode()), recvPacket.GetOpcode());
}

void WorldSession::Handle_ServerSide(WorldPacket& recvPacket)
{
    sLog->outDetail("SESSION: received server-side opcode %s (0x%.4X)", LookupOpcodeName(recvPacket.GetOpcode()), recvPacket.GetOpcode());
}

void WorldSession::Handle_Deprecated(WorldPacket& recvPacket)
{
    sLog->outDetail("SESSION: received deprecated opcode %s (0x%.4X)", LookupOpcodeName(recvPacket.GetOpcode()), recvPacket.GetOpcode());
}
