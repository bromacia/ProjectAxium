...
bool ChatHandler::HandleCooldownCommand(const char* args)
{
    ...
    else
    {
        ...
        {
            PSendSysMessage(LANG_UNKNOWN_SPELL, target == m_session->GetPlayer() ? GetAxiumString(LANG_YOU) : tNameLink.c_str());
            SetSentErrorMessage(true);
            return false;
        }

        target->RemoveSpellCooldown(spell_id, true);
        PSendSysMessage(LANG_REMOVE_COOLDOWN, spell_id, target == m_session->GetPlayer() ? GetAxiumString(LANG_YOU) : tNameLink.c_str());
   }
    return true;
}
...
bool ChatHandler::HandleAddItemCommand(const char* args)
{
    ...
    uint32 itemId = extractItemIdFromLink((char*)args);
    if (!itemId)
        return false;
    ...
    sLog->outDetail(GetAxiumString(LANG_ADDITEM), itemId, count);
...
bool ChatHandler::HandleAddItemSetCommand(const char* args)
{
    ...
    sLog->outDetail(GetAxiumString(LANG_ADDITEMSET), itemsetId);
    ...
    for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
    {
        ...
        if (skillInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                {
                    knownStr = GetAxiumString(LANG_KNOWN);
                    uint32 curValue = target->GetPureSkillValue(id);
                    uint32 maxValue  = target->GetPureMaxSkillValue(id);
                    uint32 permValue = target->GetSkillPermBonusValue(id);
                    uint32 tempValue = target->GetSkillTempBonusValue(id);

                    char const* valFormat = GetAxiumString(LANG_SKILL_VALUES);
    ...
    for (uint32 id = 0; id < sSpellMgr->GetSpellInfoStoreSize(); id++)
    {
        ...
        if (spellInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (rank)
                    ss << GetAxiumString(LANG_SPELL_RANK) << rank;
                ...
                if (talent)
                    ss << GetAxiumString(LANG_TALENT);
                if (passive)
                    ss << GetAxiumString(LANG_PASSIVE);
                if (learn)
                    ss << GetAxiumString(LANG_LEARN);
                if (known)
                    ss << GetAxiumString(LANG_KNOWN);
                if (active)
                    ss << GetAxiumString(LANG_ACTIVE);
                        ...
                        if (target)
                        {
                            ...
                            if (status == QUEST_STATUS_COMPLETE)
                            {
                                if (target->GetQuestRewardStatus(qinfo->GetQuestId()))
                                    statusStr = GetAxiumString(LANG_COMMAND_QUEST_REWARDED);
                                else
                                    statusStr = GetAxiumString(LANG_COMMAND_QUEST_COMPLETE);
                            }
                            else if (status == QUEST_STATUS_INCOMPLETE)
                                statusStr = GetAxiumString(LANG_COMMAND_QUEST_ACTIVE);
                        }
        ...
        if (factionEntry)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (repState)                               // and then target != NULL also
                {
                    ...
                    std::string rankName = GetAxiumString(index);
                    ...
                    ss << GetAxiumString(LANG_FACTION_NOREPUTATION);
        ...
        if (MapInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (MapInfo->IsContinent())
                    ss << GetAxiumString(LANG_CONTINENT);

                switch (MapInfo->map_type)
                {
                    case MAP_INSTANCE:      ss << GetAxiumString(LANG_INSTANCE);      break;
                    case MAP_BATTLEGROUND:  ss << GetAxiumString(LANG_BATTLEGROUND);  break;
                    case MAP_ARENA:         ss << GetAxiumString(LANG_ARENA);         break;
                }

                if (MapInfo->IsRaid())
                    ss << GetAxiumString(LANG_RAID);

                if (MapInfo->SupportsHeroicMode())
                    ss << GetAxiumString(LANG_HEROIC);
                ...
                if (MapInfo->IsMountAllowed())
                    ss << GetAxiumString(LANG_MOUNTABLE);
...
bool ChatHandler::HandleNearGraveCommand(const char* args)
{
    ...
    if (graveyard)
    {
        ...
        std::string team_name = GetAxiumString(LANG_COMMAND_GRAVEYARD_NOTEAM);

        if (g_team == 0)
            team_name = GetAxiumString(LANG_COMMAND_GRAVEYARD_ANY);
        else if (g_team == HORDE)
            team_name = GetAxiumString(LANG_COMMAND_GRAVEYARD_HORDE);
        else if (g_team == ALLIANCE)
            team_name = GetAxiumString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
...
bool ChatHandler::HandleListAurasCommand(const char* /*args*/)
{
    ...
    char const* talentStr = GetAxiumString(LANG_TALENT);
    char const* passiveStr = GetAxiumString(LANG_PASSIVE);
...
bool ChatHandler::HandleBanInfoCharacterCommand(const char* args)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetAxiumString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt32(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetCString(), bantime.c_str(), active ? GetAxiumString(LANG_BANINFO_YES) : GetAxiumString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
    }
    while (result->NextRow());
...
bool ChatHandler::HandleBanInfoHelper(uint32 accountid, char const* accountname)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetAxiumString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt64(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetCString(), bantime.c_str(), active ? GetAxiumString(LANG_BANINFO_YES) : GetAxiumString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
    } while (result->NextRow());
...
bool ChatHandler::HandleBanInfoIPCommand(const char* args)
{
    ...
    PSendSysMessage(LANG_BANINFO_IPENTRY,
        fields[0].GetCString(), fields[1].GetCString(), permanent ? GetAxiumString(LANG_BANINFO_NEVER) : fields[2].GetCString(),
        permanent ? GetAxiumString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[3].GetUInt64(), true).c_str(), fields[4].GetCString(), fields[5].GetCString());
...
bool ChatHandler::HandleRespawnCommand(const char* /*args*/)
{
    ...
    CellCoord p(Axium::ComputeCellCoord(player->GetPositionX(), player->GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();

    Axium::RespawnDo u_do;
    Axium::WorldObjectWorker<Axium::RespawnDo> worker(player, u_do);

    TypeContainerVisitor<Axium::WorldObjectWorker<Axium::RespawnDo>, GridTypeMapContainer > obj_worker(worker);
