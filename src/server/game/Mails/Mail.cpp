...
{
    m_messageType = MAIL_NORMAL;
    m_stationery = sender->HasGameMasterTagOn() ? MAIL_STATIONERY_GM : MAIL_STATIONERY_DEFAULT;
    m_senderId = sender->GetGUIDLow();
}
...
void MailDraft::SendMailTo(SQLTransaction& trans, MailReceiver const& receiver, MailSender const& sender, MailCheckMask checked, uint32 deliver_delay)
{
    ...
    if (sender.GetMailMessageType() == MAIL_CREATURE && sBattlegroundMgr->GetBattleMasterBG(sender.GetSenderId()) != BATTLEGROUND_TYPE_NONE)
        expire_delay = DAY;
    ...
    else
    {
        if (m_COD)
            ...
            expire_delay = pSender && pSender->HasGameMasterTagOn() ? 90 * DAY : 30 * DAY;
    }
