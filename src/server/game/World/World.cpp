...
World::AddSession_(WorldSession* s)
{
    ...
    // NOTE - CloseSession() should be called on "old" in RemoveSession()
    {
...
void World::LoadConfigSettings(bool reload)
{
    ...
    SetMotd(ConfigMgr::GetStringDefault("Motd", "Welcome to a Axium Core Server."));
    ...
    m_int_configs[CONFIG_GM_LEVEL_IN_GM_LIST]   = ConfigMgr::GetIntDefault("GM.InGMList.Level", SEC_GAMEMASTER);
    m_int_configs[CONFIG_GM_LEVEL_IN_WHO_LIST]  = ConfigMgr::GetIntDefault("GM.InWhoList.Level", SEC_GAMEMASTER);
...
void World::SetInitialWorldSettings()
{
    ...
    sLog->outString();
    sLog->outString("Loading Axium strings...");
    if (!sObjectMgr->LoadAxiumStrings())
    ...
    LoginDatabase.PExecute("UPDATE realmlist SET icon = %u, timezone = %u, revision = '%s' WHERE id = '%d'", server_type, realm_zone, _REV, realmID);      // One-time query
...
void World::SendWorldText(int32 string_id, ...)
{
    ...
    Axium::WorldWorldTextBuilder wt_builder(string_id, &ap);
    Axium::LocalizedPacketListDo<Axium::WorldWorldTextBuilder> wt_do(wt_builder);
...
BanReturn World::BanCharacter(std::string name, std::string duration, std::string reason, std::string author)
{
    ...
    if (pBanned)
        pBanned->GetSession()->CloseSession();
...
void World::ShutdownMsg(bool show, Player* player)
{
    ...
    if (show ||
        (m_ShutdownTimer < 11 && (m_ShutdownTimer % 1) == 0) || // < 10 sec; every 1 sec
        (m_ShutdownTimer < 1* MINUTE && (m_ShutdownTimer % 30) == 0) || // < 1 min; every 30 sec
